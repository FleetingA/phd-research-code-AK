...
* Diabetes diagnosed by a doctor (confounder): Converted -3s (Prefer not to answer) and -1s (Do not know) to 0s (No).

* Medication for cholesterol, blood pressure, diabetes, or take exogenous hormones: Converted -7s (None of the above) to 0s. Converted -3s and -1s to \[blank]. 

* Similar for the other medication variable excluding hormonal therapy: Converted -7s (None of the above) to 0s. Converted -3s and -1s to \[blank]. 

* Fluid intelligence/Reasoning 4: positional arithmetic: -3s and -1s set to \[blank].

* Eye problems/disorders: All -7s recoded to 0, and -1s and -3s left \[blank].

* Duration to complete numeric path (trail #1): Recoded 'trail not completed' to 0 for all such instances. 

* FI10 : arithmetic sequence recognition: Variable recoded according to UKB bespoke coding
